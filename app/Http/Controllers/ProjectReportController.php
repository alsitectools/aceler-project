    ...
    public function ajax_data(Request $request, $slug)
    {
        ...
        foreach ($projects as $project) {
            ...
            if ($project->status == 'Finished') {
                $tmp['status'] = '<span class="badge rounded-pill p-2 px-3 bg-success">' . trans("messages.Finished") . '</span>';
            } elseif ($project->status == 'Ongoing') {
                $tmp['status'] = '<span class="badge rounded-pill p-2 px-3 bg-secondary">' . trans("messages.Ongoing") . '</span>';
            } else {
                $tmp['status'] = '<span class="badge rounded-pill p-2 px-3 bg-warning">' . trans("messages.OnHold") . '</span>';
            }
            ...
            if (Auth::user()->getGuard() != 'client') {
                ...
                $tmp['action'] = '
                <a  class="action-btn btn-warning  btn btn-sm d-inline-flex align-items-center" data-toggle="popover"  title="' .  trans('messages.view_Project_Report')  . '" data-size="lg" data-title="' . __('show') . '" href="' . route(
                ...
                <a href="#" class="action-btn btn-info  btn btn-sm d-inline-flex align-items-center" data-toggle="popover"  title="' . trans('messages.Edit_Project') . '" data-ajax-popup="true" data-size="lg" data-title="' . __('Edit') . '" data-url="' . route(
                    'projects.edit',
                    [
                        $currentWorkspace->slug,
                        $project->id,
                ...
                $tmp['action'] = '
                <a  class="action-btn btn-warning  btn btn-sm d-inline-flex align-items-center" data-toggle="popover"  title="' . trans('mesages.view_Project_Report') . '" data-size="lg" data-title="' . __('show') . '" href="' . route(
                    $client_keyword . 'project_report.show',
                    [
                        $currentWorkspace->slug,
                        $project->id,
