...
<form class="" method="post" action="{{ route('notes.update',[$currentWorkspace->slug,$note->id]) }}">
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <textarea rows="3" class="form-control" id="description" name="text" placeholder="{{ __('Enter Description') }}" required>{{$note->text}}</textarea>
        </div>
        <div class="col-md-12">
        ...
        <div class="col-md-12 assign_to_selection">
            ...
            <select     id="assign_to"    name="assign_to[]" class="multi-select" data-toggle="select2" multiple="multiple" data-placeholder="{{ __('Select Users ...') }}"  >
                @foreach($users as $u)
                        <option value="{{$u->id}}" @if(in_array($u->id, $note->assign_to)) selected @endif>{{$u->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
