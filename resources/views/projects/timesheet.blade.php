            ...
            <div class="weekly-dates-div weekArrowsPadding">
                ...
                <div class="d-flex justify-content-end row1">
                    @if (isset($currentWorkspace) && $currentWorkspace)
                        @if ($project_id == -1)
                            <div class="col-sm-auto">
                                <button id="add_task" type="button" class="btn btn-primary add_task"
                                    data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Task') }}"
                                    data-url="{{ route('tasks.create', $currentWorkspace->slug) }}"
                                    title="{{ __('Add Task') }}"><i class="fa-solid fa-thumbtack"></i>
                                    {{ __('Add Task on Timesheet') }}</button>
                            </div>
                        @else
                            <div class="col-sm-auto">
                                <a href="{{ route('timesheet.csv.imputedHours.download', $project_id) }}"
