...
<div class="modal-body">
    ...
    <div class="mt-3 mb-5">
        <div class="row">
            <div class="form-group col-md-12">
                <fieldset class="custom-fieldset"
                    ...
                    <label class="statusBadge bg-warning ">{{ __('ForReview') }}</label>
                    @elseif ($milestone->status == 4)
                    <label class="bg-success statusBadge">{{ __('Finished')
                        }}</label>
                    @else
        ...
        <div class="row">
            <div class="form-group col-md-8">
                <fieldset class="custom-fieldset">
                    ...
                    <img class="imgName ps-2" @if ($salesManager->avatar) src="{{ asset($salesManager->avatar) }}"
                    @else
        ...
        <div class="row">
                    ...
                    <div class="mt-3" id="taskListContainer">
                        ...
                        <div class="taskList p-target mb-2 col-sm-12 marginText">
                            ...
                            $isLate = strtotime($task->estimated_date ?? '') < strtotime(date('Y-m-d'));
                                $dateClass=$isLate ? 'danger' : 'success' ; $icon=$isLate
                                ? '<i class="ms-2 me-2 fa-solid fa-hourglass-end fa-xs text-' . $dateClass . '"></i>'
                                : '<i class="ms-2 me-2 fa-solid fa-hourglass-start fa-xs text-' . $dateClass . '"></i>'
                                ; @endphp {!! $icon !!} {{ __($task->task_name ?? $task) }}
                        </div>
                        @endforeach
                        @else
            ...
            <div class="form-group col-md-8">
                <fieldset class="custom-fieldset pb-2">
